...
function editSeries(taskId) {
    const numSeries = prompt("Inisira um numero entre 1 e 6", "6");
...
// Função para criar um novo ID personalizado
async function criarNovoIdPersonalizado() {
    const userId = prompt("Escolha seu nome de usuário:");
    ...
    if (userId) {
        ...
        if (docSnapshot.exists) {
            alert("ID já em uso, tente outro.");
        } else {
            // Cria um novo usuário no Firestore
            await docRef.set({ id: userId, tarefas: {} }); // Tarefas será um objeto vazio inicialmente
            alert("Novo ID criado com sucesso!");
        }
    } else {
...
// Função para carregar as tarefas de um ID
async function carregarTarefas(userId) {
    if (userId) {
        ...
        if (docSnapshot.exists) {
            ...
            // Atualizar o título das tarefas de acordo com os dados do Firestore
            Object.keys(tarefas).forEach(taskId => {
                const task = document.getElementById(taskId);
                if (task && tarefas[taskId]) {
                    task.querySelector('h2').innerText = tarefas[taskId];
                }
            });

            // Exibir o userId na página
            document.querySelector('.user-id-view').innerText = `Usuário logado: ${userId}`;
...
// Função para editar o título de um exercício e salvar no Firestore
async function editExercise(taskId, originalTitle, userId) {
    const task = document.getElementById(taskId);
    const title = task.querySelector('h2'); // Pega o título da tarefa
    ...
    if (newTitle) {
        ...
        const docRef = db.collection("usuarios").doc(userId);
        await docRef.update({
            [`tarefas.${taskId}`]: newTitle // Atualiza o título da tarefa no Firestore
        });
